...
def database_cache(file):
	def decorator(func):
		...
		for name, param in sig.parameters.items():
			...
			columns.append(f"{name} {_get_nullable_type(param.annotation)}")
		try:
		...
		if is_dataclass(return_type):
			...
			columns.extend(f"return${i} {_get_nullable_type(f.type)}" for i, f in enumerate(return_fields))
			...
			columns.extend(f"return${i} {_get_nullable_type(a)}" for i, a in enumerate(args))
			return_columns = [f"return${i}" for i in range(len(args))]
			result_concat = list.extend
			get_return = lambda r: r
		else:
			columns.append(f"return {_get_nullable_type(return_type)}")
...
def _get_nullable_type(tp: type) -> str:
	origin = typing.get_origin(tp)
	# Only support unions to represent nullable types
