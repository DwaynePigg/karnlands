...
def database_cache(file):
	def decorator(func):
		...
		for name, param in sig.parameters.items():
			...
			columns.append(f"{name} {_get_sql_type(param.annotation)}")
		try:
		...
		if origin is None or origin is not tuple:
			columns.append(f"return {_get_sql_type(return_type)}")
			...
			columns.extend(f"return_{i} {_get_sql_type(a)}" for i, a in enumerate(args))
